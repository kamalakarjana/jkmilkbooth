...
class Supplier(db.Model):
    ...
    email = db.Column(db.String(120), nullable=True)  # New email field
...
def edit_supplier(supplier_id):
    ...
    s.mobile = request.form.get('mobile').strip()
...
def add_collection():
    ...
    # Send WhatsApp notification after successful collection
    if s.mobile:
        try:
...
def quick_add():
    ...
    # Send WhatsApp notification for quick add too
    if s.mobile:
        try:
            result = whatsapp_handler.send_collection_notification(s, entry)
            if result.get('success'):
...
def export_month_summary_csv():
    ...
    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow(['supplier_id','name','email','total_liters','total_amount','withdrawn','balance'])
    
    for r in rows:
        balance = int((r.total_amount or 0) - (r.withdrawn or 0))
        writer.writerow([
            r.supplier_id, r.name, r.email,
...
def send_test_whatsapp():
    ...
    test_phone = request.form.get('test_phone', '+15551477761').strip()
